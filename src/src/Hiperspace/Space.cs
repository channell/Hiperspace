            ...
            while (e >= s && p < source.Length)
            {
                ...
                switch (source[p] & btype)
                {
                    ...
                    case 2: // LEN prefixed value
                        ...
                        break;
                    case 5: // I32
                        slen = s + sizeof(Int32) < bytes.Length ? sizeof(Int32) : bytes.Length - s;
                        plen = p + sizeof(Int32) < bytes.Length ? sizeof(Int32) : bytes.Length - p;
                        len = slen < plen ? slen : plen;
                        tspan = new Span<byte>(bytes, s, len);
                        sspan = new Span<byte>(bytes, p, len);
                        sspan.CopyTo(tspan);
                        s += len;
                        p += len;
                        break;
