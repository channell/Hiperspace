module internal Hiperspace.SQL.Validation

open Hiperspace.SQL.Ast
open Hiperspace.SQL.Resolution
...
let validate (result : Result)  =
    ...
    let ``dba table`` (table : Table) =
        match table.Name with 
        | "SCHEMA_TABLES" | "SCHEMA_COLUMNS" | "SCHEMA_PROPERTIES" -> true
        | _ -> false
    ...
    and ``validate From`` (from : From) =
        ...
        | Join (a, b, c)    -> (if c.IsAJoin() then [] else [error  $"Join {c} is not an equijoin, and is not supported" from.Position]) @
                               ``validate From`` a @
                               ``validate From`` b @
                               ``validate Join`` c |> (addpos from.Position)
    ...
    and ``validate Join`` (boolean : BoolExpr) : (string * Position) list =
        match boolean with
        | And (a, b)            -> ``validate Boolean`` a @ ``validate Boolean`` b
        | Equal (a, b)          -> let inner = ``validate Expression`` a @ ``validate Expression`` b
                                   let l = a.ExpressionType
                                   ...
                                   | Some l, Some r when r.IsAssignableTo l -> inner
                                   | _ -> (error $"Expression {boolean} is not valid" nopos) :: inner
        | _                     -> [$"Expression {boolean} is not a valid join", nopos] 
